...
APP_TITLE = "Invoice Sorter – GUI (Whitelist-Filter + Export)"
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.error_rows = []   # list of {file,msg,kind}
    ...
    def _build_ui(self):
        ...
        row1 = ttk.Frame(cfg_frame); row1.pack(fill=tk.X, pady=6)
        ...
        # Row 2
        row2 = ttk.Frame(cfg_frame); row2.pack(fill=tk.X, pady=6)
        ...
        # Row 3: Ollama
        row3 = ttk.Frame(cfg_frame); row3.pack(fill=tk.X, pady=6)
        ...
        # Row 4: Dry-Run & CSV
        row4 = ttk.Frame(cfg_frame); row4.pack(fill=tk.X, pady=6)
        ...
        # Actions
        actions = ttk.Frame(root); actions.pack(fill=tk.X, pady=(0,10))
        ...
        self.btn_save.pack(side=tk.LEFT); self.btn_run.pack(side=tk.LEFT, padx=8); self.btn_stop.pack(side=tk.LEFT); self.btn_preview.pack(side=tk.RIGHT)

        # Notebook
        nb = ttk.Notebook(root); nb.pack(fill=tk.BOTH, expand=True)
        tab_log = ttk.Frame(nb); nb.add(tab_log, text="Log")
        self.progress = ttk.Progressbar(tab_log, mode="determinate", maximum=100, value=0); self.progress.pack(fill=tk.X, padx=8, pady=8)
        self.txt = tk.Text(tab_log, wrap="word", height=20); self.txt.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0,8)); self.txt.configure(state=tk.DISABLED)

        tab_prev = ttk.Frame(nb); nb.add(tab_prev, text="Vorschau")
        prev_top = ttk.Frame(tab_prev); prev_top.pack(fill=tk.X, padx=8, pady=6)
        ttk.Label(prev_top, text="Vorschau-Quelle:").pack(side=tk.LEFT)
        self.var_preview_path = tk.StringVar(); ttk.Entry(prev_top, textvariable=self.var_preview_path, width=80).pack(side=tk.LEFT, padx=6)
        ttk.Button(prev_top, text="…", command=self._preview_any_pdf).pack(side=tk.LEFT)
        self.preview_txt = tk.Text(tab_prev, wrap="word"); self.preview_txt.pack(fill=tk.BOTH, expand=True, padx=8, pady=8); self.preview_txt.configure(state=tk.NORMAL)

        tab_err = ttk.Frame(nb); nb.add(tab_err, text="Fehler")
        err_top = ttk.Frame(tab_err); err_top.pack(fill=tk.X, padx=8, pady=6)
        ...
        ttk.Checkbutton(err_top, text="Nur-Whitelist-Fails anzeigen", variable=self.var_only_wl, command=self._errors_refresh).pack(side=tk.LEFT)
        ttk.Button(err_top, text="Export angezeigte Fehler (CSV)", command=self._errors_export_csv).pack(side=tk.LEFT, padx=(12,0))
        self.err_tree = ttk.Treeview(tab_err, columns=("file","msg"), show="headings")
        self.err_tree.heading("file", text="Datei"); self.err_tree.heading("msg", text="Meldung")
        self.err_tree.column("file", width=320); self.err_tree.column("msg", width=560)
        self.err_tree.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)
        try:
            self.err_tree.tag_configure('whitelist', foreground='#b45309')  # amber
            self.err_tree.tag_configure('validation', foreground='#dc2626')  # red
        except Exception:
            pass

        tab_rx = ttk.Frame(nb); nb.add(tab_rx, text="Regex-Tester")
        rx_top = ttk.Frame(tab_rx); rx_top.pack(fill=tk.X, padx=8, pady=6)
        ...
        self.rx_text = tk.Text(tab_rx, wrap="word", height=12); self.rx_text.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0,8))
        self.rx_result = tk.Text(tab_rx, wrap="word", height=8); self.rx_result.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0,8))
        self.loaded_patterns = None
    ...
    def _choose_tesseract(self):
        f = filedialog.askopenfilename(title="tesseract.exe wählen", filetypes=[("Programme","*.exe"),("Alle Dateien","*.*")]); 
        if f: self.var_tesseract.set(f)
    def _choose_poppler(self):
        d = filedialog.askdirectory(title="Poppler bin-Ordner wählen"); 
        if d: self.var_poppler.set(d)
    def _choose_config(self):
        f = filedialog.askopenfilename(title="config.yaml wählen", filetypes=[("YAML","*.yaml;*.yml"),("Alle Dateien","*.*")])
        if f: self.var_config_path.set(f); self._load_config_silent(f)
    def _choose_patterns(self):
        f = filedialog.askopenfilename(title="patterns.yaml wählen", filetypes=[("YAML","*.yaml;*.yml"),("Alle Dateien","*.*")])
        if f: self.var_patterns_path.set(f)
    ...
    def _cfg_to_vars(self, cfg):
        ...
        if cfg.get("csv_log_path"): self.var_csv.set(True); self.var_csv_path.set(cfg.get("csv_log_path"))
    ...
    def _vars_to_cfg(self):
        ...
        if self.var_csv.get(): cfg["csv_log_path"] = self.var_csv_path.get()
        return cfg
    ...
    def _run_worker(self):
        ...
        sys.stdout = TextQueueWriter(self.queue, tag="OUT"); sys.stderr = TextQueueWriter(self.queue, tag="ERR")

        def stop_fn(): return self.stop_flag.is_set()
        def progress_fn(i, n, filename, data): self.queue.put(("PROG", (i, n, filename, data)))

        def work():
            try:
                sorter.process_all(self.var_config_path.get(), self.var_patterns_path.get(), stop_fn=stop_fn, progress_fn=progress_fn)
            except Exception as e:
    ...
    def _log(self, tag, msg):
        ...
        self.txt.insert(tk.END, f"[{ts}] {tag}: {msg}")
        ...
        try:
            while True:
                ...
                if tag == "PROG":
                    ...
                    pct = int(i / max(n, 1) * 100); self.progress.config(value=pct, maximum=100)
                    ...
                    if isinstance(payload, str):
                        ...
                        if m: self.current_file = m.group(1).strip()
    ...
    def _preview_any_pdf(self):
        path = filedialog.askopenfilename(title="PDF für Vorschau wählen", filetypes=[("PDF","*.pdf"),("Alle Dateien","*.*")])
        if not path: return
        self.var_preview_path.set(path)
        text = ""
        try:
            cfg_like = self._vars_to_cfg()
            text = sorter.extract_text_from_pdf(path, use_ocr=cfg_like.get("use_ocr", True), poppler_path=cfg_like.get("poppler_path"), tesseract_cmd=cfg_like.get("tesseract_cmd"), tesseract_lang=cfg_like.get("tesseract_lang", "deu+eng"))
        except Exception as e:
            text = f"[Fehler bei Vorschau] {e}"
        self.preview_txt.configure(state=tk.NORMAL); self.preview_txt.delete("1.0", tk.END); self.preview_txt.insert(tk.END, text[:10000]); self.preview_txt.see("1.0")
    ...
    def _load_patterns_for_tester(self):
        try:
            ...
            invn = len(pats.get("invoice_number_patterns", [])); datn = len(pats.get("date_patterns", [])); supp = len(pats.get("supplier_hints", {}) or {})
    ...
    def _run_regex_test(self):
        ...
        if not sample.strip(): messagebox.showinfo("Hinweis", "Bitte Beispieltext in das obere Feld einfügen."); return
        if not self.loaded_patterns:
            self._load_patterns_for_tester()
            if not self.loaded_patterns: return
        try:
            ...
            res = [f"Rechnungsnummer: {inv}", f"Datum: {dt.strftime('%Y-%m-%d') if dt else None}", f"Lieferant: {sup}"]
            self.rx_result.delete("1.0", tk.END); self.rx_result.insert(tk.END, "\n".join(res))
        except Exception as e:
            self.rx_result.delete("1.0", tk.END); self.rx_result.insert(tk.END, f"Fehler beim Test: {e}")
